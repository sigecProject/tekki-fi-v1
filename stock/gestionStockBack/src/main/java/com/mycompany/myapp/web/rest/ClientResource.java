...
public class ClientResource {
     ...
     */
    @GetMapping("/clients")
    public ResponseEntity<List<Client>> getAllClients(Pageable pageable, @RequestParam(required = false) String filter, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("ticket-is-null".equals(filter)) {
            log.debug("REST request to get all Clients where ticket is null");
            return new ResponseEntity<>(clientService.findAllWhereTicketIsNull(),
                    HttpStatus.OK);
        }
