...
public class TicketResource {
     ...
     */
    @GetMapping("/tickets")
    public ResponseEntity<List<Ticket>> getAllTickets(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("paiement-is-null".equals(filter)) {
            log.debug("REST request to get all Tickets where paiement is null");
            return new ResponseEntity<>(ticketService.findAllWherePaiementIsNull(),
                    HttpStatus.OK);
        }
    ...
    public ResponseEntity<List<Ticket>> searchTickets(@RequestParam String query, Pageable pageable) {
        ...
        Page<Ticket> page = ticketService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
